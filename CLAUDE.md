...
- **현재 상태**: MVP 완료 (Phase 1), Firebase 마이그레이션 준비 중 (Phase 2)
- **기술 스택**: Flutter 3.35.4, Dart 3.9.2, Hive (Phase 1), Firebase (Phase 2+)
...
6. **[TECHNICAL_DOCUMENTATION.md](./TECHNICAL_DOCUMENTATION.md)**
...
[상황] ChoreQuest는 Flutter로 개발된 가사 관리 앱입니다.
        현재 Phase 1 (Hive 로컬 DB)에서 Phase 2 (Firebase)로 마이그레이션 중입니다.
...
[참고] ARCHITECTURE.md의 "Cloud Functions" 섹션을 참고해주세요.
```
...
| 어떻게 구현하나요? | ARCHITECTURE.md, TECHNICAL_DOCUMENTATION.md | "리더보드를 어떻게 구현하나요?" |
...
#### Q3: Firebase 마이그레이션은 언제 하나요?
**A**: PRD.md Phase 2, ARCHITECTURE.md 섹션 3 참고
- Phase 2에서 우선순위 P0 (최우선)
- Hive 로컬 캐시는 유지 (오프라인 모드)
...
```
lib/
├── models/         # 데이터 모델 (Hive TypeAdapter)
├── providers/      # 상태 관리 (Provider 패턴)
├── services/       # 비즈니스 로직
├── repositories/   # 데이터 접근 (Phase 2에서 추가)
├── screens/        # UI 화면
├── widgets/        # 재사용 위젯
└── utils/          # 유틸리티
...
| 파일 | 역할 | 참고 |
|------|------|------|
| `lib/main.dart` | 앱 진입점, Provider 등록 | |
| `lib/models/user_model.dart` | User 모델, XP/레벨 로직 | TECHNICAL_DOCUMENTATION.md |
| `lib/providers/chore_provider.dart` | 집안일 상태 관리 | ARCHITECTURE.md |
| `lib/services/database_service.dart` | Hive CRUD (Phase 1) | |
| `lib/screens/home/dashboard_tab.dart` | 대시보드 화면 | |

### 5.3 데이터 플로우 예시

**집안일 완료 플로우** (ARCHITECTURE.md 섹션 4.1 참고)
```
사용자 액션: "완료" 버튼 클릭
    ↓
...
DatabaseService.completeChoreWithXp()
    ├─→ ChoreModel.complete() (Hive 저장)
    └─→ UserModel.gainXp() (XP 추가, 레벨업)
    ↓
Provider.notifyListeners()
    ↓
UI 리빌드 (XpProgressCard, LeaderboardTab)
```
...
```
Summary.md를 읽고 다음 질문에 답변해주세요.

1. ChoreQuest가 해결하려는 핵심 문제는 무엇인가요?
2. 경쟁사 Sweepy의 강점과 약점은?
3. 왜 "협업적 온보딩"이 중요한가요?
4. 한국 시장 특화 전략은 무엇인가요?
```
...
<div align="center">
  <strong>ChoreQuest AI Context Document</strong> v1.0<br>
